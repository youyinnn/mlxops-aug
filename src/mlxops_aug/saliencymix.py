...
try:
    from cv2.saliency import StaticSaliencyFineGrained_create # type: ignore
except ImportError:
    StaticSaliencyFineGrained_create = None

# Implementation Credit: https://github.com/Westlake-AI/openmixup/blob/main/openmixup/models/augments/saliencymix.py


@dataclass
class SaliencyMix(AugmentBase):

    def __post_init__(self):
        if StaticSaliencyFineGrained_create is None:
            raise ImportError(
                "opencv-contrib-python is required for SaliencyMix. "
...
@torch.no_grad()
def saliencymix(
    img, gt_label, alpha=1.0, lam=None, dist_mode=False, return_mask=False, **kwargs
):
    ...
    if not dist_mode:
        rand_index = torch.randperm(img.size(0)).to(img.device)
        ...
        if return_mask:
            mask = torch.zeros((1, 1, h, w)).to(img.device)
